...
exports.create = function (filePathPattern, keepOldFileDays) {
  ...
  var __commonHead = '', dateObj = new Date(0), day = -1, re_basenameSuffix = /^_\d{8}$/; //_YYYYMMDD
  ...
  function log(/*[OPTIONAL]tag, msg, [OPTIONAL]autoNewLine*/) {
    ...
    if (nowMs !== dateObj.valueOf()) {
      ...
      __commonHead = dpad2(dateObj.getHours()) + ':' + dpad2(dateObj.getMinutes()) + ':' + dpad2(dateObj.getSeconds()) + '.' + dpad3(dateObj.getMilliseconds()) + ' ';
      ...
      if (dateObj.getDate() !== day) {
        ...
        try {
          ...
          process.stderr.write(__commonHead + 'Failed to open/create file. ' + e + '\n');
        }
        deleteOldFiles();
      }
    }
    ...
    if (fd !== -1) {
      try {
        ok = fs.writeSync(fd, s);
      } catch (e) {
        if (!err_write) {
          err_write = true;
          process.stderr.write(head + 'Failed to append log to file. ' + e + '\n');
        }
      }
    }
  ...
  function deleteOldFiles() {
    ...
    try {
      ...
      if (e.code !== 'ENOENT') {
        process.stderr.write(__commonHead + 'Failed to search or delete old log files. ' + e + '\n');
      }
    }
