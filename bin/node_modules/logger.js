...
function __log(context, msg, option) {
  ...
  if (option && option.autoNewLine === false) {
    ...
    context.wantedNextLineOptionHead = origMsgIsEndedWithNewLine ? undefined : option.head;
  } else {
    if (context.previousLineHaveEnded) {
      s = actualHead + s + '\n';
    } else {
...
function create(filePath, keepOldFileDays) {
  var context = {filePath: filePath, keepOldFileDays: keepOldFileDays, fd: -1, previousLineHaveEnded: true};
