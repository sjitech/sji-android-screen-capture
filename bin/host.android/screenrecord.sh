...
LIB_FILE="$1"; shift || { log "require arg1: get-raw-image lib file path. Example: ../android/get-raw-image-420"; exit 1; }
...
log "--------------------------------------------------------------------------------------------------------------------------------"
log "./ffmpeg -f androidgrab -r \"$FEED_FPS\" -i \"$LIB_FILE\" -vf \"$FILTER\" -pix_fmt yuv420p \"$OUTPUT_FILE\" -y"
     ./ffmpeg -f androidgrab -r  "$FEED_FPS"  -i  "$LIB_FILE"  -vf  "$FILTER"  -pix_fmt yuv420p  "$OUTPUT_FILE"  -y
